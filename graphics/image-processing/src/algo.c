...
// Gaussian Filter
static unsigned char gf_kernel[5][5] = {
    { 2,  4,  5,  4, 2 },
    { 4,  9, 12,  9, 4 },
    { 5, 12, 15, 12, 5 },
    { 4,  9, 12,  9, 4 },
...
void* apply_grayscale(void* data, int width, int height, int bytes) {
    ...
    unsigned char* ptr = data;
    unsigned char* out = calloc(px_count, sizeof(unsigned char));

    for(int i = 0; i < bytes; i++) {
        int sum = 0;   
        for(int j = 0; j < lw_len; j++) {
            sum += ptr[i] * luminance_weights[j];
        }
        ptr[i] = sum;
    }
...
void* apply_gaussian_filter(void* data, int width, int height) {
    ...
    unsigned char* out = calloc(width * height, sizeof(*out));
    ...
    for(int i = 0; i < rows; i++) {
        for(int j = 0; j < cols; j++) {
            int sum = 0;
            for(int x = 0; x < gf_kernel_rows; x++) {
                for(int y = 0; y < gf_kernel_cols; y++) {
                    int row = i + x - row_radius;
                    int col = j + y - col_radius;
                    
                    if(row >= 0 && col >= 0 && row < rows && col < cols) {
                        int index = row * cols + col;
                        int k_index = x * gf_kernel_rows + y;
                        sum += ptr[index] * gf_kernel[x][y];
                    }   
                }
            }
...
void apply_sobel_operator(void* data, int width, int height) {
    ...
    for(int i = 0; i < rows; i++) {
        for(int j = 0; j < cols; j++) {
            int sum_so_x = 0, sum_so_y = 0;
            for(int x = 0; x < so_kernel_order; x++) {
                for(int y = 0; y < so_kernel_order; y++) {
                    int row = i + x - row_radius;
                    int col = j + y - col_radius;
                    ...
                    if(row >= 0 && col >= 0 && row < rows && col < cols) {
                        int index = row * cols + col;
                        int k_index = x * gf_kernel_rows + y;
                        sum_so_x += ptr[index] * so_kernel_x[x][y];
                        sum_so_y += ptr[index] * so_kernel_y[x][y];
                    }   
                }
            }

            int index = i * cols + j;
            out_x[index] = sum_so_x;
            out_y[index] = sum_so_y;
        }
    }
